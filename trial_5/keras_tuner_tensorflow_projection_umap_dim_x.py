...
## dim X
print('dim_x')
diffusion_dim = diffusion_temp[:,0]
...
def build_model(hp):
	...
	model.add(Dense(1)) ## change to one fore only one umap dim
...
tuner = Hyperband(
    ...
    objective='val_mse',
    executions_per_trial=2, # how many trials per variation? (same model could perform differently)
    directory='projection_umap_Hyperband_dim_x',
...
best_model.save('best_model_projection_umap_dim_x')
...
with open("tuner_projection_umap_dim_x.pkl", "wb") as f:
    pickle.dump(tuner, f)
